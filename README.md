...
Ad-blocking hosts files, IP block lists, PAC filters, and an ABP / uBO subscription, all merged, sorted and de-duped from multiple reputable sources. The generation of these lists is automated via [my homemade scripts and utilities](https://github.com/bongochong/CombinedPrivacyBlockLists/tree/master/BLT), many of which are now available in a subdirectory of this repository. Said utilities can be installed with a simple install script or one liner, for ease of use. All content in this repo is approved by Richard Stallman himself (theoretically). Use as you see fit.  
...
* [**cpbl-abp-list.txt**](https://raw.githubusercontent.com/bongochong/CombinedPrivacyBlockLists/master/cpbl-abp-list.txt): Is a simple filter subscription for AdBlock Plus and uBlock Origin, derived from my main hosts file. It works great, and is an especially good choice for those who cannot modify their hosts file for whatever reason.
