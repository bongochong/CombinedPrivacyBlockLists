...
Ad-blocking hosts files, IP block lists, PAC filters, and an ABP / uBO subscription, all merged, sorted and de-duped from multiple reputable sources. The generation of these lists is automated via [my homemade scripts and utilities, many of which are now available in a subdirectory of this repository](https://github.com/bongochong/CombinedPrivacyBlockLists/tree/master/BLT). Said utilities can be installed with a simple install script or one liner, for ease of use. All content in this repo is approved by Richard Stallman himself (theoretically). Use as you see fit.  
