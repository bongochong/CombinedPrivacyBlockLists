...
cat *.pac | sed "s/#.*$//" | sed "/^$/d" | sed "/^Site$/d" | pcregrep -f ~/BLT/parsing/tld-filter.dat | pcregrep -v -f ~/BLT/parsing/pacpatterns.dat | sed "s/^127.0.0.1 //g" | sed "s/^0.0.0.0 //g" | sed "s/^::1 //g" | sed "s/^:: //g" | sed "/ /d" | sed "s/\(.*\)/\L\1/" | sort | uniq -i > pac-uniq.txt
