...
In addition, the following packages are suggested to utilize some optional features: `curl` is necessary if you wish to use the server upload feature of the PAC generation script. `idn (or idn2)`is necessary if you wish to have any Unicode / internationalized domain names converted to ASCII Punycode entries.
